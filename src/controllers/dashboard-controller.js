...
exports.charts = async (req, res) => {
  ...
  let cantidadFichasxAnio = await FichaMedica.aggregate([
    ...
    {
      $sort: {
        //Los años obtenidos ordenarlos descendientemente
        _id: -1,
      },
    },
    {
...
async function usuario_una_ficha(anio = 0) {
  ...
  await Promise.all(
    id_fichas.map(async (x) => {
      let ficha;
      if (anio != 0) {
        ficha = await FichaMedica.find({ _id: x, anio: anio }).select({
          tipoSangre: 1,
          seguroMedico: 1,
        }); //.select({"tipoSangre":1,"seguroMedico":1})
      } else {
        ficha = await FichaMedica.find({ _id: x }).select({
          tipoSangre: 1,
          seguroMedico: 1,
        }); //.select({"tipoSangre":1,"seguroMedico":1})
      }
...
async function estadistica_tipo_asngre() {
  ...
  let tipos_sangre = await TipoSangre.find().select({
    _id: 1,
    representation: 1,
  });
  let sangre = {};
...
*/
exports.charts1 = async (req, res) => {
  let usuarios = await estadistica_tipo_asngre();

  res.status(200).json(usuarios);
};

exports.charts2 = async (req, res) => {
